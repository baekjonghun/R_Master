...
#####################################################################
class(welfare_copy$sex)

table(welfare_copy$sex)

welfare_copy$sex <- ifelse(welfare_copy$sex == 9 , NA , welfare_copy$sex)

table(is.na(welfare_copy$sex))

welfare_copy$sex <-ifelse(welfare_copy$sex == 1 , 'male' , 'female')

table(welfare_copy$sex)

qplot(welfare_copy$sex)

class(welfare_copy$income)

summary(welfare_copy$income)

qplot(welfare_copy$income)
qplot(welfare_copy$income) + xlim(0 , 1000)

welfare_copy$income <- ifelse(welfare_copy$income %in% c(0,9999) , NA , welfare_copy$income)

table(is.na(welfare_copy$income))

sex_income <- welfare_copy %>% 
  filter(!is.na(income)) %>% 
  group_by(sex) %>% 
...
########################################################################################
문제: #종교가 있는 사람들이 이혼율이 덜 할까 
########################################################################################


변수 검토 및 전처리
• 종교 (변수명 : religioin)
• 혼인상태 (변수명 : marriage) -> 코드북 참고
